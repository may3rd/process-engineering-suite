...
- All numeric fields accept bare SI numbers, `{value, unit}` objects, or strings with units (e.g., `"100 ft"`). Units are converted via `packages.hydraulics.src.utils.units`.
...
_Aliases_: `check_valve_lift` → `lift_check_valve`, `check_valve_tilting` → `tilting_check_valve`.
...
| `adjustable`                                          | When `true`, the valve drop is solved instead of treated as a fixed `pressure_drop`. You can run `run_valve_optimizer.py` (or call `packages.hydraulics.src.optimizer.optimize_control_valves`) to tune these valves so the downstream pressure matches the configured value. |
