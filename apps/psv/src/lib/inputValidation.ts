...
function validateCommon(inputs: SizingInputs): ValidationError[] {
    ...
    if (inputs.temperature === undefined || isNaN(inputs.temperature) || inputs.temperature <= -273.15) {
        errors.push({
            field: 'temperature',
 ...
 */
function validateGas(inputs: SizingInputs): { errors: ValidationError[]; warnings: ValidationError[] } {
    const errors: ValidationError[] = [];
    ...
    if (!isPositive(inputs.specificHeatRatio) || (inputs.specificHeatRatio && inputs.specificHeatRatio <= 1)) {
        errors.push({
 ...
 */
function validateLiquid(inputs: SizingInputs): { errors: ValidationError[]; warnings: ValidationError[] } {
    const errors: ValidationError[] = [];
    const warnings: ValidationError[] = [];
 ...
 */
function validateTwoPhase(inputs: SizingInputs): { errors: ValidationError[]; warnings: ValidationError[] } {
    const errors: ValidationError[] = [];
    const warnings: ValidationError[] = [];
