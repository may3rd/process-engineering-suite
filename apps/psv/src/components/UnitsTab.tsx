...
export function UnitsTab() {
  ...
  // Load areas if not already loaded (needed for area count display)
  useEffect(() => {
    if (!areAreasLoaded) {
      fetchAllAreas();
    }
  }, [areAreasLoaded, fetchAllAreas]);
  ...
  const headingTitle = selectedPlant
    ? `${selectedPlant.name} - Units`
    : "Units";
    ...
    <Box>
      ...
      {unitToDelete && (
        <DeleteConfirmDialog
          ...
          children={[{ label: "Area", count: getAreaCount(unitToDelete.id) }]}
        />
      )}
    </Box>
  );
