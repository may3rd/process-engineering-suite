...
export function ProjectsTab() {
  ...
  const headingTitle = selectedArea
  ...
  const summaryCards = useMemo(() => {
    ...
    const linkedPsvs = protectiveSystems.filter(
      (psv) => (psv.projectIds || []).length > 0,
    ).length;
    return [
      {
    ...
    <Box>
      ...
      {projectToDelete && (
        <DeleteConfirmDialog
          ...
          title={
            projectToDelete && getPsvCount(projectToDelete.id) > 0
              ? "Deactivate Project?"
              : "Delete Project?"
          }
          ...
          children={[{ label: "PSV", count: getPsvCount(projectToDelete.id) }]}
        />
      )}
    </Box>
  );
