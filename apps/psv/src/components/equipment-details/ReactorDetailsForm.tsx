...
import { VesselDetails } from '@/data/types'; // reusing VesselDetails type for now as Reactor is not explicitly typed
import { UnitSelector } from '../shared';
import { useUnitDetails, UNITS } from './hooks';

// Define ReactorDetails as VesselDetails for now, or just use VesselDetails directly
// Ideally we should export ReactorDetails from types.ts if it differs
type ReactorDetails = VesselDetails;

interface ReactorDetailsFormProps {
    details: ReactorDetails | null;
    onChange: (details: ReactorDetails) => void;
}

export function ReactorDetailsForm({ details, onChange }: ReactorDetailsFormProps) {
    const { getUnit, handleUnitChange, getDisplayValue } = useUnitDetails();

    const handleChange = (field: keyof ReactorDetails, value: any) => {
        onChange({
            ...details,
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Typography variant="subtitle2" color="primary">Reactor Geometry</Typography>
            ...
            {/* Placeholder for Reactor Internals */}
            <Typography variant="subtitle2" color="primary" sx={{ mt: 1 }}>Internals</Typography>
            <Typography variant="body2" color="text.secondary">
                Configuration for catalyst beds or heating coils can be added here.
            </Typography>
