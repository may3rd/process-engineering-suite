...
export function NotesTab() {
  ...
  const filteredTodos = todoList.filter(
    (t) => t.protectiveSystemId === selectedPsv.id && t.isActive !== false,
  );
  const filteredComments = commentList.filter(
    (c) => c.protectiveSystemId === selectedPsv.id && c.isActive !== false,
  );
  const filteredNotes = noteList.filter(
    (n) => n.protectiveSystemId === selectedPsv.id && n.isActive !== false,
  );

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleteItem, setDeleteItem] = useState<{
    id: string;
    type: "todo" | "comment" | "note";
    name: string;
  } | null>(null);
  ...
  const handleAddTask = () => {
    ...
    const dueDate =
      newTaskDueDate || defaultDueDate.toISOString().split("T")[0];
                ...
                return (
                  ...
                  >
                    ...
                    <ListItemText
                      ...
                      secondary={
                        assignee ? `Assigned to ${assignee.name}` : undefined
                      }
                    />
                  </ListItem>
                );
              ...
              return (
                ...
                >
                  <CardContent sx={{ pb: "16px !important" }}>
                    ...
                    >
                      <Box>
                        <Typography fontWeight={600}>{note.body}</Typography>
              ...
              return (
                <Card key={comment.id} variant="outlined">
                  <CardContent sx={{ pb: "16px !important" }}>
                    ...
                    >
                      <Box>
                        <Typography fontWeight={600}>{comment.body}</Typography>
                        <Typography variant="caption" color="text.secondary">
