...
import type { FlashBoilingPointType } from "@/types"
...
export async function GET(request: NextRequest) {
  ...
  if (capRaw === null) {
    const err = { error: "Query parameter 'capacity' is required" }
    return NextResponse.json(err, { status: 400 })
  }
  const capacity = Number(capRaw)
  if (!Number.isFinite(capacity) || capacity < 0) {
    const err = { error: "capacity must be a non-negative number (m³)" }
    return NextResponse.json(err, { status: 400 })
  }
  ...
  if (fpTypeRaw === null) {
    const err = { error: "Query parameter 'flashBoilingPointType' is required ('FP' or 'BP')" }
    return NextResponse.json(err, { status: 400 })
  }
  if (fpTypeRaw !== "FP" && fpTypeRaw !== "BP") {
    const err = { error: "flashBoilingPointType must be 'FP' or 'BP'" }
    return NextResponse.json(err, { status: 400 })
  }
  ...
  if (fpRaw !== null) {
    ...
    if (!Number.isFinite(parsed)) {
      const err = { error: "flashBoilingPoint must be a valid number (°C)" }
      return NextResponse.json(err, { status: 400 })
    }
