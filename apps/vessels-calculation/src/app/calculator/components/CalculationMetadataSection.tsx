...
export function CalculationMetadataSection({
  ...
  const populatedCount = useMemo(
    () => Object.values(metadata).filter((v) => v.trim().length > 0).length,
    [metadata],
  )

  const handleMetadataOpen = (open: boolean) => {
    setMetadataDialogOpen(open)
    if (open) setMetadataDraft(metadata)
  }

  const handleRevisionsOpen = (open: boolean) => {
    setRevisionDialogOpen(open)
    if (open) setRevisionsDraft(revisionHistory)
  }
            ...
            <DialogContent className="sm:max-w-4xl">
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    setRevisionsDraft((prev) =>
                      prev.length >= 3 ? prev : [...prev, { ...EMPTY_REVISION }],
                    )
                  }
              ...
              <DialogFooter className="gap-2">
                <Button type="button" variant="ghost" onClick={() => setRevisionDialogOpen(false)}>
                  Cancel
                </Button>
          ...
          <Dialog open={metadataDialogOpen} onOpenChange={handleMetadataOpen}>
            ...
            <DialogContent className="sm:max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription>
                  These fields appear in exported report headers.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid gap-3 py-2">
                {(
                  [
                    ["meta-project-number", "Project Number", "projectNumber", "e.g. PRJ-2024-001"],
                    ["meta-document-number", "Document Number", "documentNumber", "e.g. CAL-V-001"],
                    ["meta-title", "Title", "title", "Vessel volume calculation"],
                    ["meta-project-name", "Project Name", "projectName", ""],
                    ["meta-client", "Client", "client", ""],
                  ] as [string, string, keyof CalculationMetadata, string][]
                  ...
                  <div key={id} className="grid gap-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setMetadataDraft((prev) => ({ ...prev, [key]: e.target.value }))
                      }
                    />
                  </div>
                ))}
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
        <p className="text-muted-foreground">
          Project Number:{" "}
          <span className="text-foreground">{metadata.projectNumber || "—"}</span>
        </p>
        <p className="text-muted-foreground">
          Document Number:{" "}
          <span className="text-foreground">{metadata.documentNumber || "—"}</span>
        </p>
        <p className="text-muted-foreground">
        ...
        <p className="text-muted-foreground">
          Project Name:{" "}
          <span className="text-foreground">{metadata.projectName || "—"}</span>
        </p>
        <p className="text-muted-foreground">
        ...
        <p className="text-muted-foreground">
          Revision Records:{" "}
          <span className="text-foreground">{revisionHistory.length}</span>
        </p>
      </div>
