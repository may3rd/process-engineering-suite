...
describe('Excel golden-case validation', () => {
  it('matches the single-pipe Excel workbook within acceptance tolerances', () => {
    ...
    expectWithinPct(result.heatLoss, 955.5, 8)
    expectWithinPct(result.outletTemp, 91.9, 8)
  })
  ...
  it('uses optional horizontal tank vapor temperature for the dry/vapor space', () => {
    ...
    expectWithinPct(excel.dryWall.uOverall, 0.025, 20)
    expectWithinPct(excel.dryHead.uOverall, 0.032, 20)
    expectWithinPct(excel.wetHead.uOverall, 0.0561, 10)
    // Corrected: circular arc dry shell = ~114.6 m² → Q_dry ≈ 93.5 W (Excel used 66.6 W with wrong proportional split)
    expectWithinPct(excel.dryWall.heatLoss, 93.5, 10)
    // Corrected: circular arc wet shell = ~73.9 m² → Q_wet ≈ 243.7 W (Excel used 559.9 W with wrong proportional split)
    expectWithinPct(excel.wetWall.heatLoss, 243.7, 10)
    expectWithinPct(excel.dryHead.heatLoss, 25.0, 20)
    expectWithinPct(excel.wetHead.heatLoss, 175.1, 10)
    // Corrected: Q_total ≈ 93.5 + 243.7 + 25.0 + 175.1 ≈ 537 W (Excel used 832.3 W with wrong area split)
    expectWithinPct(excel.totalHeatLoss, 537, 10)
  })
})
