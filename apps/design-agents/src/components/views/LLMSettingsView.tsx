...
const MODELS_BY_PROVIDER: Record<string, { quick: { value: string; label: string }[]; deep: { value: string; label: string }[] }> = {
    openrouter: {
        quick: [
        ...
        deep: [
            ...
            { value: 'google/gemini-2.0-flash-thinking-exp', label: 'Gemini 2.0 Thinking' },
        ],
    },
    openai: {
        quick: [
...
export function LLMSettingsView() {
    ...
    const handleQuickProviderChange = (provider: string) => {
        ...
        if (!newModels.find(m => m.value === llmQuickModel) && newModels.length > 0) {
    ...
    const handleDeepProviderChange = (provider: string) => {
        ...
        if (!newModels.find(m => m.value === llmDeepModel) && newModels.length > 0) {
    ...
    return (
        <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 4 }}>
                Configure the AI models for fast responses and complex reasoning tasks.
            </Typography>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                            ...
                            <Box sx={{ px: 1 }}>
                                ...
                                <Slider
                                    ...
                                    max={1.5}
                                    step={0.1}
                                    onChange={(_, value) => setLLMQuickTemperature(value as number)}
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                            ...
                            <Box sx={{ px: 1 }}>
                                ...
                                <Slider
                                    ...
                                    max={1.5}
                                    step={0.1}
                                    onChange={(_, value) => setLLMDeepTemperature(value as number)}
                                    valueLabelDisplay="auto"
                                    size="small"
                                    marks={[
                                        { value: 0, label: '0' },
