...
def build_subtype_row_values(
    ...
    if equipment_type == "tank":
        return {
            ...
            "latitude_deg": details.get("latitude"),
            "working_temperature": details.get("workingTemperature"),
            "working_temperature_unit": details.get("workingTemperatureUnit"),
            "fluid": details.get("fluid"),
            "vapour_pressure": details.get("vapourPressure"),
            "vapour_pressure_unit": details.get("vapourPressureUnit"),
            "flash_point": details.get("flashPoint"),
            "flash_point_unit": details.get("flashPointUnit"),
            "boiling_point": details.get("boilingPoint"),
            "boiling_point_unit": details.get("boilingPointUnit"),
            "latent_heat": details.get("latentHeat"),
            "latent_heat_unit": details.get("latentHeatUnit"),
            "relieving_temp": details.get("relievingTemperature"),
            "relieving_temp_unit": details.get("relievingTemperatureUnit"),
            "molecular_weight_g_mol": details.get("molecularWeight"),
            "tank_configuration": details.get("tankConfiguration"),
            "insulation_conductivity_w_mk": details.get("insulationConductivity"),
            "inside_heat_transfer_coeff_w_m2k": details.get("insideHeatTransferCoeff"),
            "insulated_surface_area_m2": details.get("insulatedSurfaceArea"),
            "extra": extra,
        }
...
def build_details_from_subtype_row(equipment_type: str, row: Any) -> dict[str, Any] | None:
    ...
    if equipment_type == "tank":
        base = {
            ...
            "latitude": _as_float(row.latitude_deg),
            "workingTemperature": _as_float(row.working_temperature),
            "workingTemperatureUnit": row.working_temperature_unit,
            "fluid": row.fluid,
            "vapourPressure": _as_float(row.vapour_pressure),
            "vapourPressureUnit": row.vapour_pressure_unit,
            "flashPoint": _as_float(row.flash_point),
            "flashPointUnit": row.flash_point_unit,
            "boilingPoint": _as_float(row.boiling_point),
            "boilingPointUnit": row.boiling_point_unit,
            "latentHeat": _as_float(row.latent_heat),
            "latentHeatUnit": row.latent_heat_unit,
            "relievingTemperature": _as_float(row.relieving_temp),
            "relievingTemperatureUnit": row.relieving_temp_unit,
            "molecularWeight": _as_float(row.molecular_weight_g_mol),
            "tankConfiguration": row.tank_configuration,
            "insulationConductivity": _as_float(row.insulation_conductivity_w_mk),
            "insideHeatTransferCoeff": _as_float(row.inside_heat_transfer_coeff_w_m2k),
            "insulatedSurfaceArea": _as_float(row.insulated_surface_area_m2),
        }
        compact = {k: v for k, v in base.items() if v is not None}
        if row.insulated is not None:
            compact["insulated"] = row.insulated
