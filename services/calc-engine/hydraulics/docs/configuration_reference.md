...
- All numeric fields accept bare SI numbers, `{value, unit}` objects, or strings with units (e.g., `"100 ft"`). Units are converted via `hydraulics.utils.units`.
...
| `adjustable`                                          | When `true`, the valve drop is solved instead of treated as a fixed `pressure_drop`. You can run `run_valve_optimizer.py` (or call `hydraulics.optimizer.optimize_control_valves`) to tune these valves so the downstream pressure matches the configured value. |
